...
import cn.topcheer.pms2.dao.pml.PageDao;
...
public class PageService extends GenericService<PmlGrid> {
    ...
    public PageDao getPageDao() {
        return (PageDao) this.getGenericDao();
    }
    @Autowired
    public void setPageDao(PageDao pageDao) {
        this.setGenericDao(pageDao);
    }
     ...
     * @return
     */
    public Page<Map> findPageBySql(String sql, List paramList, JSONObject pageConfig){
        ...
        if(pageConfig.has("sorts")){
            if(!Util.isEoN(pageConfig.get("sorts"))){
                ...
                if(sorts.size()>0){
                    ...
                    sql = "select * from ("+ sql +") where 1=1 " + orderBySql;
                }
            }
        }
        //获取当前分页下的数据
        ...
        return new Page<Map>(list, Integer.parseInt(totalCount));
    }
     ...
     * @return
     */
    public Page<Map> findPageByHql(String hql, List paramList, JSONObject pageConfig){
        //通过前台传来的currentPage：当前第几页;pageSize：每页多少数据，
        //计算出获取所有数据当中改的范围值，bigNum:范围最大值，smallNum:范围最小值
        int pageSizes = Integer.parseInt(pageConfig.get("pageSize").toString());//每页多少数据
        int currentPage = Integer.parseInt(pageConfig.get("currentPage").toString());//当前第几页
        int startNum =(currentPage-1)*pageSizes;//从多少数据开始
        //获取数据的总数
        String countHql = "select count(*) "+hql.substring(hql.indexOf("from"),hql.length());
        Query countQuery = this.getPageDao().getQuery(countHql);
        if(paramList.size()>0){
        ...
        //获取当前分页下的数据
        Query hqlQuery = this.getPageDao().getQuery(hql);
        if(paramList.size()>0){
            for (int i = 0; i <paramList.size() ; i++) {
     ...
     * @return
     */
    public Page<Map> findPageBySqlForRownum(String sql, List paramList, JSONObject pageConfig){
        //通过前台传来的currentPage：当前第几页;pageSize：每页多少数据，
        //计算出获取所有数据当中改的范围值，bigNum:范围最大值，smallNum:范围最小值
        int bigNum = Integer.parseInt(pageConfig.get("currentPage").toString())*Integer.parseInt(pageConfig.get("pageSize").toString())+1;
        int smallNum = (Integer.parseInt(pageConfig.get("currentPage").toString())-1)*Integer.parseInt(pageConfig.get("pageSize").toString());
     ...
     * @return
     */
    public Page<Map> findPageBySqlForRownumArs(String sql, List paramList, JSONObject pageConfig){
        //通过前台传来的currentPage：当前第几页;pageSize：每页多少数据，
        //计算出获取所有数据当中改的范围值，bigNum:范围最大值，smallNum:范围最小值
        int bigNum = Integer.parseInt(pageConfig.get("currentPage").toString())*Integer.parseInt(pageConfig.get("pageSize").toString())+1;
        int smallNum = (Integer.parseInt(pageConfig.get("currentPage").toString())-1)*Integer.parseInt(pageConfig.get("pageSize").toString());
