...
@Service
public class RcSbLimitStrategy extends AbstractLimitStrategy {

    @Resource
    private PmsTalentService pmsTalentService;
    ...
    public void executeConfigCondition(List<Map<String, String>> limitConfigConditionList, HashMap<String, Object> requestParameter) {
        ...
        PmsTalent pmsTalent = pmsTalentService.findById(mainId);
        boolean isSb = !Util.isEoN(pmsTalent);
    ...
    private void judgeSbNum(boolean isSb, String limitConfigParam, String batchId) {
        ...
        HqlBuilder<PmsTalent> builder = HqlBuilder.builder();
        builder.eq(PmsTalent::getPlanprojectbatchid, batchId);
        builder.eq(PmsTalent::getDeclarantid, AuthUtil.getUserId());
        builder.eq(PmsTalent::getIsDeleted, 0);
        Long count = pmsTalentService.findCount(builder);
